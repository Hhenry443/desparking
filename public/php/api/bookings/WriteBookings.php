...
class WriteBookings extends Bookings
{
    ...
    ");
        ...
        // Fetch user email so it's stored on the booking row
        $emailStmt = $this->db->prepare("SELECT user_email FROM users WHERE user_id = :id LIMIT 1");
        $emailStmt->execute([':id' => $userID]);
        $emailRow  = $emailStmt->fetch(PDO::FETCH_ASSOC);
        $userEmail = $emailRow['user_email'] ?? null;
